...
while True:
    ...
    resized_frame = imresize(img, (img_height, img_width), preserve_range=True)
    resized_frame = np.array([resized_frame])

    predictions = model.predict(resized_frame)

    valid_predictions = predictions[0, predictions[0, :, 1] > 0]

    for pred in valid_predictions:
        class_id, confidence, xmin, ymin, xmax, ymax = pred

        # Transform to original frame
        xmin = int(xmin * (img.shape[1] / img_width))
        xmax = int(xmax * (img.shape[1] / img_width))
        ymin = int(ymin * (img.shape[0] / img_height))
        ymax = int(ymax * (img.shape[0] / img_height))

        cv2.rectangle(img, (xmin, ymin), (xmax, ymax), (0, 255, 0), 2)

        # Insert predicted class label to the bounding box
        class_label = classes[int(class_id)]
        cv2.putText(img, class_label, (xmin, ymin - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
